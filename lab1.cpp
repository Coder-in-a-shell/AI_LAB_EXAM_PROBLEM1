...
class iterate
{
  ...
  double gamma = 0.9, theta = 0.0001;
    vector < float >prob =
    { 1, 0.99, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0 }, randoms;
  int itr = 0, cnt = 0;
public:
  ...
  pair < int, double >dfs (Node * temp, int currState)
  {
    ...
    double answerReward = 0, mx = 0, oldValue =
      ValueFunction[currState], quitReward = ValueFunction[currState - 1];
    pair < int, double >nextStateReward;
    if (randoms[itr++ % 10000000] <= prob[currState])
      {

	nextStateReward = dfs (temp->next, currState + 1), answerReward =
	  prob[currState] * (gamma * nextStateReward.second + temp->val),
	  maxStateReached =
	  max (maxStateReached, nextStateReward.first),
	  ValueFunction[currState] =
	  (ValueFunction[currState] * TimesVisited[currState] +
	   answerReward) / (TimesVisited[currState] + 1);
    ...
    return
    {
    maxStateReached, max (quitReward, answerReward)};
  }
